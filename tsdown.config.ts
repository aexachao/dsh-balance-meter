 ...
 * a <style data-plugin> tag at factory execution.
 */
...
/**
 * The loader module table the harness shares into the frozen browser module
 * table (packages/client/web/src/platform.ts). Only these may stay external;
...
/**
 * Documented TEMPORARY exemption in the harness preset: the snapshot-store
 * engine lives in the runtime pending its promotion-time rehoming. At runtime
 * the lazy CJS table answers the require natively.
 */
const RUNTIME_STORE_EXEMPTION = '@deepseek-ai/dsh-client-runtime/client'

const CLIENT_EXTERNALS: readonly string[] = [...PLATFORM_MODULES, RUNTIME_STORE_EXEMPTION]

/** Vendored framework libraries: rescoped into @deepseek-ai, inline-safe. */
...
function sourceAssetPath(source: string, importer: string): string {
  ...
  // tsc emits the client program under lib/client/; the css sources live
  // under src/client/. Rewrite the emitted prefix back to src.
  const marker = `${sep}lib${sep}client${sep}`
  return emitted.replace(marker, `${sep}src${sep}client${sep}`)
}
