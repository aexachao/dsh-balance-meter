...
export function parsePeakWindows(spec) {
    ...
    for (const part of spec.split(',')) {
        ...
        if (startMin >= 24 * 60 || endMin > 24 * 60 || startMin >= endMin)
            continue;
...
export function offPeakWindows(windows) {
    const sorted = [...windows].sort((a, b) => a.startMin - b.startMin);
    const gaps = [];
    let cursor = 0;
    for (const w of sorted) {
        if (w.startMin > cursor)
            gaps.push({ startMin: cursor, endMin: w.startMin });
        if (w.endMin > cursor)
            cursor = w.endMin;
    }
    if (cursor < 24 * 60)
...
export function isPeak(timeMs, windows, timeZone) {
 ...
 */
export function isDeepSeekProvider(provider, model) {
    const p = provider?.toLowerCase();
    const m = model?.toLowerCase();
    if (p !== undefined && p.includes('deepseek'))
        return true;
    if (m !== undefined && m.includes('deepseek'))
        return true;
    if (p !== undefined || m !== undefined)
        return false; // known non-DeepSeek identity
 ...
 */
export function deepseekProfile(model) {
...
export function costOf(tokens, profile, peak) {
